...
class SCAN(Node):
    ...
    def __init__(self):
        ...
        time_period_2 = time_period_1*4
        # self.timer_2 = self.create_timer(time_period_2,self.timer_2_callback)
...
##########################################################################################################################################################################      
        def polar_to_xy():
            ...
            y = []
            i = 0
            for i in range(len(self.lidar_msgs.ranges)):
                angle_increment = (abs(self.lidar_msgs.angle_min - self.lidar_msgs.angle_max))/len(self.lidar_msgs.ranges)

                current_angle_increment = angle_increment*(i+1)

                current_angle = self.lidar_msgs.angle_min + current_angle_increment

                point_x = float(self.lidar_msgs.ranges[i]) * (math.cos(current_angle))
                point_y = -(float(self.lidar_msgs.ranges[i]) * (math.sin(current_angle)))

                theta = -math.pi/2
                x_r = (point_x*math.cos(theta)) + (point_y*math.sin(theta)) + self.T_x_lidar_baselink
                y_r = (point_y*math.cos(theta)) - (point_x*math.sin(theta)) + self.T_y_lidar_baselink
...
##########################################################################################################################################################################  
        ...
        self.stack_theta_vertex[2] = self.stack_theta_vertex[1]
        self.stack_theta_vertex[1] = self.stack_theta_vertex[0]

        self.stack_distance_vertex[3] = self.stack_distance_vertex[2]
        ...
        if theta_vertex != 99 :
            ...
            self.stack_theta_vertex[0] =  theta_vertex
        ...
        # print((self.stack_theta_vertex[0]))
        # print((self.stack_theta_vertex[1]))
        # print((self.stack_theta_vertex[2]))
